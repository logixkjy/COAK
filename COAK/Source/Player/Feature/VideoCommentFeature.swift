    ...
    func reduce(into state: inout State, action: Action) -> Effect<Action> {
        ...
        case .postComment:
            ...
            return .run { [videoId = state.videoId, userId = state.userId, email = state.email] send in
                let comment = try await commentClient.postComment(videoId, text, userId, email)
        ...
        case let .postReply(parentId):
            ...
            return .run { [videoId = state.videoId, userId = state.userId, email = state.email] send in
                let reply = try await commentClient.postReply(videoId, parentId, text, userId, email)
